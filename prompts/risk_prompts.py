...
# Mode Selection Message Template
MODE_SELECTION_MESSAGE = """Great! Let's define your risk profile. You have two options:
...
# Direct Equity Confirmation Message Template
def get_direct_equity_message(equity: float) -> str:
    """Generate confirmation message for direct equity setting."""
    bond = 1.0 - equity
...
# Review/Edit Message Template
def get_review_edit_message(equity: float) -> str:
    """Generate review/edit message showing current allocation."""
    bond = 1.0 - equity
...
# Questionnaire Question Template
def get_questionnaire_question_template() -> str:
    """Get the template for questionnaire questions."""
    return """Reply with the option number (e.g., '2'), or say 'I pick the second one'. If unsure, say 'why?'."""

# Questionnaire Finalization Message Template
def get_questionnaire_finalization_message(equity: float, bond: float, answers: dict) -> str:
    """Generate final message after questionnaire completion."""
...
# Error Messages
UNKNOWN_INTENT_MESSAGE = """I'm not sure what you'd like to do. You can:
...
PROCEED_WITHOUT_RISK_MESSAGE = "Please set your risk allocation first before proceeding."

INVALID_EQUITY_MESSAGE = "Please provide an equity allocation between 0.05 and 0.95 (e.g., 0.70 for 70%)."

NO_RISK_ALLOCATION_MESSAGE = "No risk allocation set yet. Please set your equity allocation first."

UNKNOWN_QUESTIONNAIRE_RESPONSE_TEMPLATE = "I didn't understand your response. Please choose an option:\n\n{question_text}\n\n{options}"
